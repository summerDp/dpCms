...
<div ng-controller="enterpriseTestCtrl">
		...
		<tbody>
			<tr id="tr{{item.id}}" ng-repeat-start="item in $data">
				...
				<td data-title="'所属性质'" filter="{ 'property': 'select' }"
					...
					sort-indicator="'span'">
					{{dictMap[item.dictionary.id]}}
				</td>
				<td data-title="'传真'" filter="{ 'fax_like': 'text' }"
					sortable="'fax'" sort-indicator="'span'">
					{{item.fax}}
				</td>
				<td data-title="'邮箱'" filter="{ 'emailAddress_like': 'text' }"
					sortable="'emailAddress'" sort-indicator="'span'">
					{{item.emailAddress}}
				</td>
				<td data-title="'电话'" filter="{ 'telephone_like': 'text' }"
					sortable="'telephone'" sort-indicator="'span'">
					{{item.telephone}}
				</td>
	...
	<script type="text/ng-template" id="ng-table/headers/checkbox.html">
		<input type="checkbox" ng-model="checkboxes.checked" id="checkboxes_select_all" name="filter-checkbox" value="" />
	</script>
